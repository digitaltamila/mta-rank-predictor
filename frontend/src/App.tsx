...
function MyResultsPanel({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    if (profile) void loadResults(profile.mobile, profile.sessionToken)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const handleSendOtp = async (mob: string) => {
    if (!/^[6-9]\d{9}$/.test(mob)) { setError('Enter a valid 10-digit Indian mobile number.'); return }
    setError(null); setSending(true)
    try { await sendOtp(mob); setOtpSent(true) }
    catch (e) { setError(e instanceof Error ? e.message : 'Failed to send OTP.') }
    finally { setSending(false) }
  }

  const handleResendOtp = async () => {
    setOtp(''); setError(null); setSending(true)
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const newProfile = { mobile: res.mobile, sessionToken: res.session_token }
      saveProfile(newProfile)
      setProfile(newProfile)
      setSessionExpired(false)
      setOtpSent(false); setMobile(''); setOtp('')
  ...
  const handleLogout = () => {
    clearProfile()
    setProfile(null); setResults(null); setSessionExpired(false)
    setMobile(''); setOtp(''); setOtpSent(false); setError(null)
  }

  const showLoginForm = !profile || sessionExpired
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center" role="dialog" aria-modal>
      ...
      <div className="relative w-full max-w-md rounded-t-2xl bg-surface sm:rounded-2xl shadow-[0_24px_60px_rgba(17,24,39,0.2)] border border-border max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 grid gap-4">
          ...
          {/* Profile card — always shown when logged in */}
          ...
          {sessionExpired && profile && (
            <div className="rounded-lg border border-yellow/40 bg-yellow/10 p-4">
              <p className="text-sm font-semibold text-foreground">Session expired for {profile.mobile}</p>
              <p className="mt-0.5 text-xs text-muted-foreground">Re-verify your number to continue.</p>
              <button
                type="button"
                className="mt-2 text-xs font-bold text-navy underline underline-offset-2"
                onClick={() => { setMobile(profile.mobile); setSessionExpired(false) }}
              >
                Re-verify →
              </button>
            </div>
          ...
          {/* Login / OTP form — only when no profile or session expired */}
          {showLoginForm && (
            <div className="grid gap-3">
              ...
              <div className="flex gap-2">
                ...
                />
                {!otpSent && (
                  <Button type="button" variant="secondary" disabled={sending || mobile.length < 10} onClick={() => handleSendOtp(mobile.trim())}>
          ...
          {/* Results list */}
          {!loading && results !== null && !sessionExpired && (
            <div className="grid gap-3">
              {error && <p className="text-sm font-medium text-red">{error}</p>}
              {results.length === 0 && !error && (
                <p className="py-6 text-center text-sm text-muted-foreground">
                  No results yet. Submit your response sheet URL to get started.
                </p>
              )}
              {results.map((r) => (
